...
START_TEST(create_test) {
    o_forward_list_t* list = o_list_create(int);
...
START_TEST(push_front_test) {
    o_forward_list_t* list = o_list_create(unsigned);
    unsigned val = 31111U;
    ...
    ck_assert_uint_eq(*(int*)list->front->data, val + 1);
...
START_TEST(clear_test) {
    ...
    o_forward_list_push_front(list, &a);
    o_forward_list_push_front(list, &a);
    ck_assert_int_eq(*(int*)list->front->next->next->next->data, a);
